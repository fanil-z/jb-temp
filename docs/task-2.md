...
A better-planned roadmap and structured changes eventually bring more value than chaotic immediate action. Especially in large ecosystems. Investigation and preparation take time, but they will help us understand the scope of required changes and identify the real causes of existing issues.

Within this step, I would:

- make an **inventory of target documentation, data, and metrics**;
- make a table of documented sections with their owners and received feedback;
- prepare an action plan and involve other stakeholders.

## **Step 1. Standardization and Single-Source-of-Truth**

From the given list, I would start by addressing inconsistency in terminology and metric definitions, as this is a high-impact issue. **Standardization and implementing the single-source-of-truth approach** in the department would also solve most of the problems that employees are currently facing.

If different teams maintain the documentation independently, then it is time to bring them together and align the way of working. Considering that documentation quality varies, a simple solution is to take the best parts of the documentation as a standard and apply them to other sections. The process with the agreed standard should be documented and communicated to the contributing teams. It would be beneficial to organize 2-3 workshops to kick off the standardization and cleanup processes.

!!! warning

    This type of documentation improvement is positive and should be received well by contributors. However, in some cases, standardization creates tension between teams. Considering that we eventually need to decide on ONE solution and deprecate or rewrite the work of other teams, we need to think of ways to preserve good team dynamics and not let involved parties feel discouraged or left out.
...
As a next step, I would address frequent questions in chat, as this is a low-hanging fruit.

I personally think that a technical writer should be an active member of internal chats and forums related to their features and docs. Forums and chats are a great source of feedback and a way to connect with users.
...
5. [Optional] After cleaning up the data, it can be beneficial to invest in a **vector search engine** (semantic or similarity search). The vector search database can later be used to implement an **Ask AI** feature. A simple semantic search can be built using open-source solutions. I have experience with **Qdrant**-based search, and the implementation was not difficult. However, it requires a server for the engine and the vector DB.

!!! info
    
    This step partially intersects with Step 1. Labeling outdated pages and improving information architecture can be done in parallel within organized workshops.
...
- There is clear ownership of documentation sections.
